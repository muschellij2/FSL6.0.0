...
public:
  ...
  virtual boost::shared_ptr<MISCMATHS::SpMat<float> > J() const;

  virtual boost::shared_ptr<MISCMATHS::SpMat<float> > J(const std::vector<unsigned int>&  deriv) const;

  virtual boost::shared_ptr<MISCMATHS::SpMat<float> > J(const NEWIMAGE::volume<float>&    ima,
							const NEWIMAGE::volume<char>      *mask) const;

  virtual boost::shared_ptr<MISCMATHS::SpMat<float> > J(const std::vector<unsigned int>&  deriv,
							const NEWIMAGE::volume<float>&    ima,
							const NEWIMAGE::volume<char>      *mask) const;
  ...
  virtual boost::shared_ptr<MISCMATHS::SpMat<float> > BendEnergyHessAsSpMat() const;
...
private:
  ...
  boost::shared_ptr<MISCMATHS::SpMat<float> > get_j(const Spline3D<double>&            sp,
						    const std::vector<unsigned int>&   csz,
						    const float                        *ima,
  ...
  void calculate_memen_bender_H(const std::vector<unsigned int>&   lksp,
				const std::vector<unsigned int>&   csz,
				const std::vector<unsigned int>&   isz,
				EnergyType                         et,
				unsigned int*&                     irp,
