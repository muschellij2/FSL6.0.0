...
boost::shared_ptr<SpMat<float> > splinefield::J() const
{
  std::vector<unsigned int> deriv(3,0);
  NEWIMAGE::volume<float>   ones(static_cast<int>(FieldSz_x()),static_cast<int>(FieldSz_y()),static_cast<int>(FieldSz_z())); ones = 1.0;
  boost::shared_ptr<SpMat<float> > tmp = J(deriv,ones,NULL);
  return(tmp);
}

boost::shared_ptr<SpMat<float> > splinefield::J(const std::vector<unsigned int>&  deriv) const
{
  NEWIMAGE::volume<float>   ones(static_cast<int>(FieldSz_x()),static_cast<int>(FieldSz_y()),static_cast<int>(FieldSz_z())); ones = 1.0;
  boost::shared_ptr<SpMat<float> > tmp = J(deriv,ones,NULL);
  return(tmp);
...
boost::shared_ptr<SpMat<float> > splinefield::J(const NEWIMAGE::volume<float>&    ima,						   
						const NEWIMAGE::volume<char>      *mask) const
{
  std::vector<unsigned int>  deriv(3,0);
  boost::shared_ptr<SpMat<float> >  tmp = J(deriv,ima,mask);
  return(tmp);
}

boost::shared_ptr<SpMat<float> > splinefield::J(const std::vector<unsigned int>&  deriv,
						const NEWIMAGE::volume<float>&    ima,						   
						const NEWIMAGE::volume<char>      *mask) const
{
  if (deriv.size() != 3) throw BasisfieldException("splinefield::J: Wrong size derivative vector");
  if (mask != NULL && !samesize(ima,*mask)) throw BasisfieldException("splinefield::J: Mismatch between image and mask");
  ...
  boost::shared_ptr<SpMat<float> > tmp;
...
boost::shared_ptr<BFMatrix> splinefield::MemEnergyHess(MISCMATHS::BFMatrixPrecisionType   prec) const  // Hessian of membrane energy
{
  ...
  calculate_memen_bender_H(lksp,csz,isz,MemEn,irp,jcp,valp);
...
boost::shared_ptr<BFMatrix> splinefield::BendEnergyHess(MISCMATHS::BFMatrixPrecisionType   prec) const // Hessian of bending energy
{
  ...
  unsigned int *irp; unsigned int *jcp; double *valp;
  calculate_memen_bender_H(lksp,csz,isz,BendEn,irp,jcp,valp);
  boost::shared_ptr<BFMatrix>  H;
  if (prec==BFMatrixFloatPrecision) H = boost::shared_ptr<BFMatrix>(new SparseBFMatrix<float>(ncoef,ncoef,irp,jcp,valp));
  else H = boost::shared_ptr<BFMatrix>(new SparseBFMatrix<double>(ncoef,ncoef,irp,jcp,valp));
  delete [] irp; delete [] jcp; delete [] valp;  
...
boost::shared_ptr<MISCMATHS::SpMat<float> > splinefield::BendEnergyHessAsSpMat() const // Hessian of bending energy
{
  std::vector<unsigned int>    lksp(3,0);
  lksp[0] = Ksp_x(); lksp[1] = Ksp_y(); lksp[2] = Ksp_z();
  std::vector<unsigned int>    csz(3,0);
  ...
  unsigned int *irp; unsigned int *jcp; double *valp;
  calculate_memen_bender_H(lksp,csz,isz,BendEn,irp,jcp,valp); // N.B. irp, jcp and valp passed by reference
  boost::shared_ptr<MISCMATHS::SpMat<float> > H = boost::shared_ptr<MISCMATHS::SpMat<float> >(new SpMat<float>(ncoef,ncoef,irp,jcp,valp));
  delete [] irp; delete [] jcp; delete [] valp;  
...
boost::shared_ptr<SpMat<float> > splinefield::get_j(const Spline3D<double>&            sp,
						    const std::vector<unsigned int>&   csz,
						    const float                        *ima,
						    const std::vector<unsigned int>&   isz) const
{
  ...
  boost::shared_ptr<SpMat<float> > j = boost::shared_ptr<SpMat<float> >(new SpMat<float>(m,n,irp,jcp,valp));
...
//
void splinefield::calculate_memen_bender_H(const std::vector<unsigned int>&  lksp,
					   const std::vector<unsigned int>&  csz,
					   const std::vector<unsigned int>&  isz,
					   EnergyType                        et,
					   unsigned int*&                    irp,
