...
public:
  ...
  // The NzMaxInA calculates the maximum # of non-zero elements of A where
  // A implements a 3D spline basis as a matrix.
  unsigned int NzMaxInA(const std::vector<unsigned int>&  isz) const;
...
/////////////////////////////////////////////////////////////////////
//
// Get maximum # of non-zero elements in A/J
//
/////////////////////////////////////////////////////////////////////

template<class T>
unsigned int Spline3D<T>::NzMaxInA(const std::vector<unsigned int>&  isz) const
{
  std::vector<unsigned int>  ci(3,0);   // Coefficient/spline index
  std::vector<unsigned int>  fi(3,0);    // First index into field
  std::vector<unsigned int>  li(3,0);    // Last index into field

  unsigned int nzmax = 0;
  for (ci[2]=0; ci[2]<_vsp1D[2].NCoef(isz[2]); ci[2]++) {
    for (ci[1]=0; ci[1]<_vsp1D[1].NCoef(isz[1]); ci[1]++) {
      for (ci[0]=0; ci[0]<_vsp1D[0].NCoef(isz[0]); ci[0]++) {
	this->RangeInField(ci,isz,fi,li);
	nzmax += (li[2]-fi[2])*(li[1]-fi[1])*(li[0]-fi[0]);
      }
    }
  }
  return(nzmax);  
