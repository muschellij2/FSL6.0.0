...
string csfPriorName, grayPriorName, whitePriorName;
  ...
  if(bapused>0)
	...
	read_volume(pCSF, csfPriorName);
      ...
      if(FslFileExists(grayPriorName))
	read_volume(pGM, grayPriorName);
      ...
      if(FslFileExists(whitePriorName))
	...
	system(reg);
      ...
      if(bapused>0)
      {
	if(FslFileExists((main_prior_vol+"_csf_stdspace")))
	...
	if(FslFileExists(main_prior_vol+"_gm_stdspace"))
	...
	if(FslFileExists(main_prior_vol+"_wm_stdspace"))
