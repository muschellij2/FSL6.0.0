...
class MelodicOptions {
 ...
 inline MelodicOptions::MelodicOptions() :
   ...
   joined_vn(string("--sep_vn"), true,
   	   string("switch on separate variance nomalisation for each input dataset (off by default)"),
       false, no_argument),
   dr_pca(string("--mod_pca"), true,
	   string("switch off modified PCA for concat ICA"),
	   false, no_argument, false),
   migp(string("--disableMigp"), true,
	   string("switch off MIGP data reduction when using -a concat (full temporal concatenation will be used)"),
	   false, no_argument),
   ...
   numICs(string("-n,--numICs"), -1,
	   ...
	   false, requires_argument),
   approach(string("-a,--approach"),  string("concat"),
	   string("approach for multi-session/subject data:\n"
"\t   concat\ttemporally-concatenated group-ICA using MIGP ( default )\n"
"\t   tica  \ttensor-ICA"),
	   false, requires_argument),

   deflation(string("--defl"),false,
	   string("switch single-subject ICA method from symmetric to deflation"),
	     false, no_argument,false),
   nonlinearity(string("--nl"), string("pow3"),
	   string("\tnonlinearity: gauss, tanh, pow3 (default), pow4"),
	   false, requires_argument),
   ...
   debug(string("--debug"),  false,
	   ...
	   false, no_argument),
   readCIFTI(string("--CIFTI"),  false,
	   string("        input/output as CIFTI (warning: auto-dimensionality estimation for CIFTI data is currently inaccurate)"),
	   false, no_argument),
