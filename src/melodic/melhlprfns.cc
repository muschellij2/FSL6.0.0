...
namespace Melodic{
  ...
  void update_mask(volume<float>& mask, Matrix& Data)
  {
	...
	int nOriginalRows(Data.Nrows()), nOriginalColumns(Data.Ncols());
	int64_t nMaskVoxels=newmaskM.Sum();
	if ( nMaskVoxels < newmaskM.Ncols() ) { //Then there are some unmasked voxels, so we need to reformat
	  newData.ReSize(nOriginalRows,nMaskVoxels);
	  int64_t newColumn(1);
	  for ( int64_t originalColumn=1;originalColumn<=nOriginalColumns;originalColumn++)
...
//	cerr << " " <<tmpD2.Ncols() << " " << N << " " << dim << endl;
 ...
 void adj_eigspec(const RowVector& in, RowVector& out1, RowVector& out2)
  ...
  Matrix ppca_est(const RowVector& eigenvalues, const int N)
  {
    ...
    RowVector tmp1;
    tmp1 = -Melodic::cumsum(eigenvalues).Reverse()+sum(eigenvalues,2).AsScalar();
    tmp1(1) = 0.95*tmp1(2);
    tmp1=tmp1.Reverse();

    RowVector tmp2;
    tmp2 = -Melodic::cumsum(logLambda).Reverse()+sum(logLambda,2).AsScalar();
    tmp2(1)=tmp2(2);
    tmp2=tmp2.Reverse();
    ...
    RowVector l_lam;
    l_lam = -(N/2)*Melodic::cumsum(logLambda);
    ...
    RowVector l_RRN;
    l_RRN = -0.5*N*SP(k,log(SP(Melodic::cumsum(eigenvalues),pow(k,-1))))+l_nu;
  ...
  void basicGLM::olsfit(const Matrix& data, const Matrix& design, 
	...
	dof = ols_dof(design);
      Matrix pinvModel(pinv(design)); // inverted model used several times
      beta =  pinvModel*data;
	...
	for(int64_t col=1;col<=t.Ncols();col++)
	  for (int64_t row=1;row<=t.Nrows();row++) {
	    z(row,col)=t2z.convert(t(row,col),dof,&logp);
	    p(row,col)=exp(logp);
	  }
      }
    }
