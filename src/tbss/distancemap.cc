...
Option<string> secondname(string("--secondim"), string(""),
		  string("second image filename (calc closest distance of this and primary input image, using non-zero voxels, negative distances mean this secondary image is the closer one)"),
		  false, requires_argument);
...
int do_work(int argc, char* argv[]) 
{
  ...
  if (secondname.set()) {
    read_volume(vsecond,secondname.value());
  }
  ...
  if (valueimname.set()) {
    ...
    if (maskname.set()) {
      if (secondname.set()) {
	dmap = distancemap(vin,vsecond,mask);
      } else {
  ...
  if (labelname.set() || segmentname.set()) {
    if (secondname.set()) { cerr << "At present cannot use either localmax or segment options together with secondary image" << endl; exit(EXIT_FAILURE); }
