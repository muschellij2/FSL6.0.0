		...
		const volume<float>& outvol)
{
  ...
  for (int z=outvol.minz(); z<=outvol.maxz(); z++) {
	...
	warpvol(x,y,z,0) = xin(1);
	warpvol(x,y,z,1) = xin(2);
	warpvol(x,y,z,2) = xin(3);
      }
    }
  }
  return 0;
		 ...
		 volume4D<float>& totalwarp)
{
  ...
  for (int z=postwarp.minz(); z<=postwarp.maxz(); z++) {
	...
	xmid(1) = postwarp(x,y,z,0);
	xmid(2) = postwarp(x,y,z,1);
	xmid(3) = postwarp(x,y,z,2);
	...
	totalwarp(x,y,z,0) = xpre(1);
	totalwarp(x,y,z,1) = xpre(2);
	totalwarp(x,y,z,2) = xpre(3);
      }
    }
  }
  return 0;
...
volume<float> calc_sigloss(volume4D<float>& lrgrad, float te, float gammabar)
{
  ...
  for (int z=lrgrad.minz(); z<=lrgrad.maxz(); z++) {
	...
	float sincl = Sinc(gbarte_2*lrgrad(x,y,z,0));
	float sincr = Sinc(gbarte_2*lrgrad(x,y,z,1));
	float thetal = M_PI*gbarte_2*lrgrad(x,y,z,0);
	float thetar = M_PI*gbarte_2*lrgrad(x,y,z,1);
...
void grad_calc(volume4D<float>& gradvols, const volume4D<float>& warp)
{
  ...
  for (int z=warp.minz(); z<=warp.maxz(); z++) {
	...
	if ((z<warp.maxz()) && (y<warp.maxy()) && (x<warp.maxx())) {
	  ...
	  */
	  gradvols(x,y,z,0) = (wx100-wx000)/dx;
	  gradvols(x,y,z,1) = (wx010-wx000)/dx;
	  gradvols(x,y,z,2) = (wx001-wx000)/dx;
	  gradvols(x,y,z,3) = (wy100-wy000)/dy;
	  gradvols(x,y,z,4) = (wy010-wy000)/dy;
	  gradvols(x,y,z,5) = (wy001-wy000)/dy;
	  gradvols(x,y,z,6) = (wz100-wz000)/dz;
	  gradvols(x,y,z,7) = (wz010-wz000)/dz;
	  gradvols(x,y,z,8) = (wz001-wz000)/dz;
	} else {
...
 	  gradvols[0](x,y,z) = (warp[0](x2,y,z) - warp[0](x,y,z))/dx;
	  ...
	  */
	  gradvols(x,y,z,0) = (warp(x2,y,z,0) - warp(x,y,z,0))/dx;
	  gradvols(x,y,z,1) = (warp(x,y2,z,0) - warp(x,y,z,0))/dx;
	  gradvols(x,y,z,2) = (warp(x,y,z2,0) - warp(x,y,z,0))/dx;
	  gradvols(x,y,z,3) = (warp(x2,y,z,1) - warp(x,y,z,1))/dy;
	  gradvols(x,y,z,4) = (warp(x,y2,z,1) - warp(x,y,z,1))/dy;
	  gradvols(x,y,z,5) = (warp(x,y,z2,1) - warp(x,y,z,1))/dy;
	  gradvols(x,y,z,6) = (warp(x2,y,z,2) - warp(x,y,z,2))/dz;
	  gradvols(x,y,z,7) = (warp(x,y2,z,2) - warp(x,y,z,2))/dz;
	  gradvols(x,y,z,8) = (warp(x,y,z2,2) - warp(x,y,z,2))/dz;
	}
      }
    }
  }
			      ...
			      float warpmeanx, float warpmeany, float warpmeanz)
{
  ...
  for (int n=0; n<3; n++) {
    ...
    ShadowVolume<float> gradkre0(gradkre[0]),gradkre1(gradkre[1]),gradkre2(gradkre[2]);
    ShadowVolume<float> gradkim0(gradkim[0]),gradkim1(gradkim[1]),gradkim2(gradkim[2]);
    warpfns_fft3(grad[n*3+0],gradkre0,gradkim0);
    warpfns_fft3(grad[n*3+1],gradkre1,gradkim1);
    warpfns_fft3(grad[n*3+2],gradkre2,gradkim2);
	...
	for (int x=grad.minx(); x<=grad.maxx(); x++) {
	  ...
	  gradkrealx = gradkre(x,y,z,0);
	  gradkimagx = gradkim(x,y,z,0);
	  gradkrealy = gradkre(x,y,z,1);
	  gradkimagy = gradkim(x,y,z,1);
	  gradkrealz = gradkre(x,y,z,2);
	  gradkimagz = gradkim(x,y,z,2);
	  ...
	  if (fabs(norm)>1e-12) {
	    gradkre(x,y,z,0) = dotprodre / norm;
	    gradkim(x,y,z,0) = dotprodim / norm;
	  } else {
	    gradkre(x,y,z,0) = 0;
	    gradkim(x,y,z,0) = 0;
	  }
    ...
    ShadowVolume<float> newwarpn(newwarp[n]);
    warpfns_ifft3(gradkre0,gradkim0,newwarpn);
  }
		...
		float minJ, float maxJ, const Matrix& p_initaffmat)
{
  ...
  for (int z=grad.minz(); z<=grad.maxz()-1; z++) {
	...
	for (int jacnum=0; jacnum<8; jacnum++) {
	  if ((jvol(x,y,z,jacnum)<minJ) || (jvol(x,y,z,jacnum)>maxJ)) {
		...
		J(n1,n2) = grad(x+xoff[n2],y+yoff[n2],z+zoff[n2],3*n1+n2-4);
	    } }
	    alpha = 0.0;
	    Jnew = J;
...
// 	      }
	    ...
	    for (int n1=1; n1<=3; n1++) { for (int n2=1; n2<=3; n2++) {
		grad(x+xoff[n2],y+yoff[n2],z+zoff[n2],3*n1+n2-4) = 
