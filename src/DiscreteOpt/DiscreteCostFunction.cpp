...
namespace DISCRETEOPT{
  ...
  void DiscreteCostFunction::initialize(int numNodes,int numLabels, int numPairs, int numTriplets, int numQuartets)
  {
    ...
    if (m_num_triplets != numTriplets || m_num_labels != numLabels)
     {
       cout << m_num_triplets << " m_num_Triplets " << numTriplets << endl;

	if (tripletcosts) delete [] tripletcosts;
	if (tripletweights) delete [] tripletweights;
	tripletcosts = new double[numTriplets*numLabels*numLabels*numLabels];
	tripletweights = new double[numTriplets*numLabels*numLabels*numLabels]; // not used? delete?
     }
    
    if (m_num_quartets!= numQuartets || m_num_labels != numLabels)
     {
       cout << m_num_quartets << " m_num_Quartets " << numQuartets <<  endl;

       if (quartetcosts){ cout << "delete Quartet costs " << endl;delete [] quartetcosts; cout << numQuartets << " " <<numLabels <<  " after delete " <<numQuartets*numLabels*numLabels*numLabels*numLabels <<  endl; }
	quartetcosts = new double[numQuartets*numLabels*numLabels*numLabels*numLabels];
     }
    */
    ...
    // std::fill(tripletcosts,tripletcosts+m_num_labels*m_num_labels*m_num_labels*m_num_triplets,0.0f);
    //  std::fill(tripletweights,tripletweights+m_num_labels*m_num_labels*m_num_labels*m_num_triplets,0.0f);
    // std::fill(quartetcosts,quartetcosts+m_num_labels*m_num_labels*m_num_labels*m_num_labels*m_num_quartets,0.0f);
  ...
  void DiscreteCostFunction::reset()
  {
    ...
    //if(m_is_memory) m_paircosts_memory.clear();
    /*if(tripletcosts) std::fill(tripletcosts,tripletcosts+m_num_labels*m_num_labels*m_num_labels*m_num_triplets,0.0f);
    if(tripletweights) std::fill(tripletweights,tripletweights+m_num_triplets,0.0f);
    if(quartetcosts) std::fill(quartetcosts,quartetcosts+m_num_labels*m_num_labels*m_num_labels*m_num_labels*m_num_quartets,0.0f);
    */
    //if(m_is_memory_triplets) m_tripletcosts_memory.clear();
  }
  ...
  /*
  //====================================================================================================================//
  void DiscreteCostFunction::initTripletWeights(const double *weights)
  {
    if(!tripletweights) tripletweights = new double[m_num_triplets];

    if(weights)
      {
	for(int p = 0; p < m_num_triplets; ++p)
	  {
	    tripletweights[p] = weights[p];
	  }
      }
    else
	{
	  std::fill(tripletweights,tripletweights+m_num_triplets,1.0f);
	}
  }
  */  
  //====================================================================================================================//
  /*void CostFunction::enablePairwiseMemory()
  {
    //clear the precomputed distance memory
    m_paircosts_memory.clear();
    m_paircosts_memory.resize(m_num_pairs);
    for(int pair = 0; pair < m_num_pairs; ++pair)
      m_paircosts_memory[pair].resize(m_num_labels*m_num_labels);
    m_is_memory = true;
  }

  //====================================================================================================================//
  void CostFunction::enableTripletMemory()
  {
    //clear the precomputed distance memory
    m_tripletcosts_memory.clear();
    m_tripletcosts_memory.resize(m_num_triplets);
    for(int triplet = 0; triplet < m_num_triplets; ++triplet)
      m_tripletcosts_memory[triplet].resize(m_num_labels*m_num_labels*m_num_labels);
    m_is_memory_triplets = true;
  }
  */
  //====================================================================================================================//
  ...
  double DiscreteCostFunction::evaluateTotalCostSum(const int *labeling, const int *pairs, const int *triplets, const int *quartets)
  {
    ...
    if(_debug)  cout << m_num_quartets << " cost_sum_unary " << cost_sum_unary << " cost_sum_pairwise " << cost_sum_pairwise  << " cost_sum_triplet " << cost_sum_triplet << " cost_sum_quartet " << cost_sum_quartet <<" total " <<  cost_sum_unary + cost_sum_pairwise + cost_sum_triplet + cost_sum_quartet << " m_num_triplets 2 " << m_num_triplets <<  endl;
 ...
 //====================================================================================================================//
  ...
  /* void  NonLinearSRegDiscreteCostFunction::computeTripletCosts(){
    ...
    for(int i=0;i<_CPgrid.ntriangles();i++){

      
      // for each node triplet calculate face angles and estimate difference
      
      for(int labelA=0; labelA<_labels.size();labelA++){
	for(int labelB=0; labelB<_labels.size();labelB++){
	  for(int labelC=0; labelC<_labels.size();labelC++){
	    tripletcosts[i*m_num_labels*m_num_labels*m_num_labels+labelC*m_num_labels*m_num_labels+labelB*m_num_labels+ labelA]=  computeTripletCost(i,labelA,labelB,labelC);
	    if(   tripletcosts[i*m_num_labels*m_num_labels*m_num_labels+labelC*m_num_labels*m_num_labels+labelB*m_num_labels+ labelA] > maxval) maxval=tripletcosts[i*m_num_labels*m_num_labels*m_num_labels+labelC*m_num_labels*m_num_labels+labelB*m_num_labels+ labelA];
	  }
	}
      }
 ...
 //====================================================================================================================//
  ...
  void SegmentationCostFunction::set_parameters(myparam & ALLPARAMS){

    myparam::iterator it;
    it=ALLPARAMS.find("lambda1");_lambda1=boost::get<float>(it->second);
    it=ALLPARAMS.find("lambda2");_lambda2=boost::get<float>(it->second);
    it=ALLPARAMS.find("bscount");_BScount=boost::get<int>(it->second); 
    it=ALLPARAMS.find("verbosity");_verbosity=boost::get<bool>(it->second);
    it=ALLPARAMS.find("outdir");_outdir=boost::get<string>(it->second);
 ...
 //====================================================================================================================//
  void SegmentationCostFunction::initialize(int numNodes, int numLabels, int numPairs, int numTriplets, int numQuartets)
  {
    DiscreteCostFunction::initialize(numNodes,numLabels,numPairs,numTriplets,numQuartets);
  }
  
  /*void  SegmentationCostFunction::computeTripletCosts(){
   
    double maxval=0;

    for(int i=0;i<_SOURCE.ntriangles();i++){

      
      // for each node triplet calculate face angles and estimate difference
      
      for(int labelA=0; labelA<2;labelA++){
	for(int labelB=0; labelB<2;labelB++){
	  for(int labelC=0; labelC<2;labelC++){
	    tripletcosts[i*m_num_triplets*m_num_triplets+labelC*m_num_triplets+labelB*m_num_triplets+ labelA]=computeTripletCost(i,labelA,labelB,labelC);
	    if(tripletcosts[i*m_num_triplets*m_num_triplets+labelC*m_num_triplets+labelB*m_num_triplets+ labelA] > maxval) maxval=tripletcosts[i*m_num_triplets*m_num_triplets+labelC*m_num_triplets+labelB*m_num_triplets+ labelA];
	  }
	}
      }
    ...
    cout << " max triplet " << maxval << endl;
  }
  */
  double SegmentationCostFunction::computeTripletCost(int triplet, int labelA, int labelB, int labelC){
  
    if(labelA!=labelB || labelA!=labelC){return _lambda1; }
    else return 0;
  }

  double  SegmentationCostFunction::computePairwiseCost(int pair, int labelA, int labelB){

    if(labelA!=labelB) return _lambda2;
    else return 0;
  
   
  }

  void  SegmentationCostFunction::computePairwiseCosts(const int *pairs)
  {
    int pair=0;
    ...
    while(pair<m_num_pairs){
      ...
      for(int j=0;j<m_num_pairs;j++){

      	for(int k=0;k<m_num_pairs;k++){
  ...
  double SegmentationCostFunction::computeUnaryCost(int node, int label){

    int index = node%_SOURCE.nvertices()+1;
    int subject=floor(node/_SOURCE.nvertices());

  
    if((subject >=(int)_DATA.size()) ||(index>_DATA[subject].Ncols())) {    throw  DISCRETEOPTHOCRException("group segmentation CostFunction::(subject >=DATA.size()) ||(index>_DATA[subject].Ncols())");}
    
      return 1-_DATA[subject](label+1,index);
   
  }

  void  SegmentationCostFunction::computeUnaryCosts(){
  ...
  void MetricDistortionDiscreteCostFunction::initialize(int numNodes, int numLabels, int numPairs, int numTriplets, int numQuartets)
  {
    DiscreteCostFunction::initialize(numNodes,numLabels,numPairs,numTriplets,numQuartets);
  }

  double MetricDistortionDiscreteCostFunction::computeTripletCost(int triplet, int labelA, int labelB, int labelC){
    vector<int> id;
    Pt normal;
    vector<Pt> v(_SPHERE.nvertices(), Pt());
    Pt v0,v1,v2;
    double W; 
    ...
    v0=_SPHERE.get_triangle_vertex(triplet,0);
    v1=_SPHERE.get_triangle_vertex(triplet,1);
    v2=_SPHERE.get_triangle_vertex(triplet,2);
      id.push_back(_SPHERE.get_triangle_vertexID(triplet,0));
    id.push_back(_SPHERE.get_triangle_vertexID(triplet,1));
    id.push_back(_SPHERE.get_triangle_vertexID(triplet,2));
    ...
    _SPHERE.set_coord(id[0],v[id[0]]);
    _SPHERE.set_coord(id[1],v[id[1]]);
    ...
    _SPHERE.set_coord(id[0],v0);
    _SPHERE.set_coord(id[1],v1);
    _SPHERE.set_coord(id[2],v2);
