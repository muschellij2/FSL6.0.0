...
// 	cout<< "tcontrast" << tcontrast<< endl;
	...
	return ret;
      ...
      void readFileAsVolume(const string& filename, volume4D<float>& target, const volume<float>& mask);
      void saveVolumeAsFile(const volume<float>& source, const string& filename, const volume<float>& mask);
    private:
