...
namespace GSA {
  ...
  private:
    vector<uniqueNode>& dataMap;
    list<uniqueNode> myNodes;
  };

  template <class T>
  class CiftiSurfaceGeometry : AbstractGeometry<T> {
  public:
  CiftiSurfaceGeometry(const vector<cifti::CiftiBrainModelsMap::SurfaceMap>& inputMap) : dataMap(inputMap) {};
    virtual std::string type() {return "SURFACE";}
  private:
    vector<cifti::CiftiBrainModelsMap::SurfaceMap> dataMap;
  };
  ...
  VolumeGeometry(const NEWIMAGE::volume<T> inputGeometry, const std::vector<uniqueNode>& fullMap,const std::vector<uniqueNode>& myMap): geometry(inputGeometry) {};
    NEWIMAGE::volume4D<T> asVolume(const Matrix& data);
    NEWMAT::ReturnMatrix asMatrix(const NEWIMAGE::volume4D<T>& data);
    void updateMatrix(const NEWIMAGE::volume4D<T>& source, Matrix& target);
    virtual std::string type() {return "VOLUME";}
    void maskMap(const NEWIMAGE::volume<T>& mask);
    ReturnMatrix updateMapAndMatrix(const NEWIMAGE::volume<T>& mask, const Matrix& data);
...
{
public:
  ReturnMatrix abstractFile(const string&filename,const string& mode);
  void saveMatrix(const Matrix& data, const string& filename);
  void remapData(Matrix& data); 
