...
void fnirt_CF::common_construction(std::vector<boost::shared_ptr<BASISFIELD::basisfield> >& pdf_field)
{
  ...
  *robj = 0.0;
  ...
  df = boost::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(RefSz_x(),RefSz_y(),RefSz_z(),df_field.size()));
  df->copyproperties(*ssvref); //for header info
  for (unsigned int i=0; i<df_field.size(); i++) {
    NEWIMAGE::ShadowVolume<float> tmpVol((*df)[i]);
    //(*df)[i] = 0.0;
    tmpVol=0.0;
    df_field[i]->AsVolume(tmpVol);
...
void fnirt_CF::ForceJacobianRange(double minj, double maxj) const
{
  ...
  for (unsigned int v=0; v<3; v++) {
	...
	for (unsigned int i=0; i<Nx; i++) {
          defvol(i,j,k,v) = df_field[v]->PeekWide(i-xoff,j-yoff,k-zoff);
	}
      }
    }
  }
  ...
  for (unsigned int i=0; i<3; i++) {
    NEWIMAGE::ShadowVolume<float> tmpVol(defvol[i]);
    add_affine_part(AffineMat(),i,tmpVol);
  }
  ...
  for (unsigned int i=0; i<3; i++) {
    NEWIMAGE::ShadowVolume<float> tmpVol(defvol[i]);
    remove_affine_part(AffineMat(),i,tmpVol);
  }
  ...
  for (unsigned int v=0; v<3; v++) {
    for (unsigned int k=0; k<RefSz_z(); k++) {
      for (unsigned int j=0; j<RefSz_y(); j++) {
	for (unsigned int i=0; i<RefSz_x(); i++) {
          cvol(i,j,k,v) = defvol(i+xoff,j+yoff,k+zoff,v);
	}
      }
    }
  }
...
const NEWIMAGE::volume4D<float>& fnirt_CF::RobjDeriv() const
{
  if (!robj_deriv) { // If first time derivative of object image is requested
    ...
    robj_deriv->copyproperties(*ssvref);
    robj_deriv_updated = false;
  }
...
void fnirt_CF::SetDefField(const NEWIMAGE::volume4D<float>& vfield) const
{
  ...
  for (unsigned int i=0; i<df_field.size(); i++) {
    ...
    NEWIMAGE::ShadowVolume<float> tmpVol((*df)[i]);
    df_field[i]->AsVolume(tmpVol);   // N.B. may not be identical to vfield[i]
  }
  robj_updated = false;
  robj_deriv_updated = false;
  if (objmask) {
