...
{
  return vector2affine(params,n,Globaloptions::getInstance().impair->testCog,aff);
}
...
int affmat2vector(const Matrix& aff, int n, ColumnVector& params)
 {
   ...
   return affmat2vector(aff,n,Globaloptions::getInstance().impair->testCog,params);
 }
...
void fix2D(volume<float>& vol) 
{
 ...
 void eval_costs(volume<float>& refvol, volume4D<float>& timeseries, vector<Matrix>& mat_array, float current_scale) {
   ...
   for (int i=0; i < globalopts. no_volumes; i++){
     ShadowVolume<float> tempVol(timeseries[i]);
     usrsetscale(refvol, tempVol, (int)current_scale);
...
int main (int argc,char** argv)
{
  ...
  for (int mean_its = 0; mean_its < 1 + globalopts. meanvol; mean_its++) {
	...
	for (int i = 0; i < globalopts. no_volumes; i++){	  
	  testvol = timeseries[i];
	  timeseries.setextrapolationmethod(extraslice);
	  timeseries.setinterpolationmethod(trilinear);
  ...
  for (int i=0; i < globalopts. no_volumes; i++){
    ...
    if (globalopts. sinc_final) {
      timeseries.setextrapolationmethod(extraslice);
      timeseries.setinterpolationmethod(sinc);
      affine_transform(timeseries[i],testvol,mat_array0[i]*init_trans,1.0);
    } else  if (globalopts. nn_final) {
      timeseries.setextrapolationmethod(extraslice);
      timeseries.setinterpolationmethod(nearestneighbour);
      affine_transform(timeseries[i],testvol,mat_array0[i]*init_trans,1.0);
    } else  if (globalopts. spline_final) {
      timeseries.setextrapolationmethod(extraslice);
      timeseries.setinterpolationmethod(spline);
      ShadowVolume<float> temp(timeseries[i]);
      temp.forcesplinecoefcalculation();
      affine_transform(temp,testvol,mat_array0[i]*init_trans,1.0);
      //timeseries.setextrapolationmethod(extraslice);
      //timeseries.setinterpolationmethod(spline);
      //affine_transform(timeseries[i],testvol,mat_array0[i]*init_trans,1.0);
    } else {
  ...
  save_volume_dtype(timeseries, globalopts.outputfname, globalopts.datatype);
}
