...
optionMenu2 $w.statsMode.type fmri(mixed_yn) -command "feat5:updatestats $w 0; feat5:setMinimumInputs" 3 "Fixed effects" 0 "Mixed effects: Simple OLS" 2 "Mixed effects: FLAME 1" 1 "Mixed effects: FLAME 1+2" 4 "Randomise"

balloonhelp_for $w.statsMode.type "The main choice here is between fixed effects (FE) and mixed effects (ME) higher-level modelling. FE modelling is more \"sensitive\" to activation than ME, but is restricted in the inferences that can be made from its results; because FE ignores cross-session/subject variance, reported activation is with respect to the group of sessions or subjects present, and not representative of the wider population. ME does model the session/subject variability, and it therefore allows inference to be made about the wider population from which the sessions/subjects were drawn.
...
pack $w.statsMode.type -side left

set fmri(randomisePermutations) 5000
LabelSpinBox $w.statsMode.perms -label "Permutations" -textvariable fmri(randomisePermutations) -range {1 1000000 }  
...
optionMenu2 $w.thresh.menu fmri(thresh) -command "feat5:updatepoststats $w" 0 "None" 1 "Uncorrected" 2 "Voxel" 3 "Cluster" 4 "TFCE"
