...
volume4D<float> acs;
  ...
  if ( prewhiten ) {
    prewhiten=false;
    if(FslImageExists(featdir+"/stats/threshac1")) {
  ...
  for(int type=0;type<2;type++) /* setup stats type */
  {
	...
	read_volume(imcope,featdir+"/stats/cope"+num2str(i));
      ...
      /* If we're plotting zstats, figure out if the current contrast is empty (i.e. all zeros) */
      if (type == 0 && !modelFree) {
        double consum = 0;

        for (int j = numEVs * (i - 1); j < numEVs * i; j++) {
          consum += abs(normalisedContrasts[j]);
        }
        
        if (consum == 0 || !isfinite(consum))
          emptyContrast = true;
      }
      ...
      if (!useCoordinate) {
	if (!customMask) {
	  if (FslImageExists(featdir+"/cluster_mask_"+statType+num2str(i)))
	...
	for(int x=0; x<im.xsize(); x++) 
	  for(int y=0; y<im.ysize(); y++) 
		...
		maskedVoxels++;
		double weight(1);
		if (v!=0 && zWeightClusters && (!emptyContrast)) weight=imz(x,y,z);
	...
	if (useTriggers)
	{
	  ...
	  for(int ev=0; ev<numEVs; ev++) 
	    if (triggers[ev]>0.5) {
	      ...
	      if (!(modelFree || emptyContrast)) ps_compact.ReSize((int)(10*ps_period)+1,6);
		...
		for(int t=int_min_t+1;t<=max_t;t++)
		{
		  ...
		  if (!(modelFree || emptyContrast)) input << (ceil((t-min_t-1)*10))/10.0 << TS_residuals(t)+TS_model(t) << TS_model(t) << TS_pemodel(ev*npts+t) << TS_residuals(t)+TS_pemodel(ev*npts+t) << 1;  //(restricted temporal accuraccy (0.1*TR) must be at least 0.1 ( scatter can not take 0 )
		  else input << t-min_t-1 << TS_data(t) << 1;
		...
		peristimulusText+="<td>Full model fit - <a href=\""+graphName+"p.png\">Partial model fit</a> - <a href=\""+graphName+".txt\">Raw data</a><br>\n<IMG BORDER=0 SRC=\""+graphName+".png\">\n";
	      ...
	      else
	      {
           ...
           Matrix blank=ps_full.SubMatrix(1,ps_full.Nrows(),1,2).t();
           blank=log(-1.0);
		...
		newplot.add_label("");
		newplot.add_label("data");
		newplot.timeseries(blank,graphFileName,title,-0.1,PSSIZE,3,2,false);
      newplot.deletescatter();
      
		newplot.remove_labels(3);
		peristimulusText+="Data plot - <a href=\""+graphName+".txt\">Raw data</a>\n<IMG BORDER=0 SRC=\""+graphName+".png\"><br><br>\n";
	      }
	      newplot.remove_xlabel();
	    }
	  if (!(modelFree || emptyContrast)) peristimulusText+="</tr></table><br><br>\n";
	}
