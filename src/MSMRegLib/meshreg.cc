...
namespace MESHREG {
  ...
  void MeshReg::Initialize_level(int level)
  {
	...
	MODEL->set_anatomical_neighbourhood(ANAT_to_CP_baryweights,ANAT_face_neighbourhood);
      ...
      }else{if(_regmode==3){throw  MeshReg_error("STRAINS based regularisation requires anatomical meshes");}
  ...
  void MeshReg::saveTransformedData(const double &sigma, const string &filename){
    ...
    if(_anat){
      ...
      newmesh STRAINSmesh=calculate_triangular_strains(in_anat,ANAT_TRANS);
  ...
  NEWMESH::newmesh MeshReg::project_CPgrid(NEWMESH::newmesh SPH_in,NEWMESH::newmesh  REG, int num){ /// num indices which warp for group registration
   ...
   if(_level==1){
      ...
      MODEL->warp_CPgrid(MESHES[num],transformed_mesh,num); // for tri clique model control grid is continously deformed
      }
   }else {
     ...
     MODEL->warp_CPgrid(icotmp,REG,num); 
