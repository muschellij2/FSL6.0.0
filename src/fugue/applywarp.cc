...
int ApplyWarpHelper::applywarp()
{
  ...
  for (unsigned int t=0; t<(unsigned int) invol.tsize(); t++) {
    ...
    ShadowVolume<float> tmpOutVol(outvol[t]);
    invol.setpadvalue(invol[t].backgroundval());
    invol.setextrapolationmethod(extraslice);
    // do the deed
    if (superflag) {
      apply_warp(invol[t],affmat,defvol,currentPostmat,currentPremat,*ssout_ptr);
      if (interp.value() == "nn") ApplyWarpHelper::downsample(*ssout_ptr,ssvec,true,tmpOutVol);
      else ApplyWarpHelper::downsample(*ssout_ptr,ssvec,false,tmpOutVol);
    }
    else {
      apply_warp(invol[t],affmat,defvol,currentPostmat,currentPremat,tmpOutVol);
    }
    outvol[t] *= involmask;
  ...
  // save the results
  if (datatype.set()) {
    save_volume_dtype(outvol,outname.value(),dtypecode);
  }
  else {
    if (dtype(inname.value()) != DT_FLOAT && dtype(inname.value()) != DT_DOUBLE) { // If integery
      if (ApplyWarpHelper::range(outvol) < 100) save_volume_dtype(outvol,outname.value(),DT_FLOAT);
      else save_volume_dtype(outvol,outname.value(),dtype(inname.value()));
    }
    else save_volume_dtype(outvol,outname.value(),dtype(inname.value()));
  }
