...
volume<float> initialise_invwarp(volume4D<float>& invwarp, const volume4D<float>& warpvol)
{
  ...
  if (debug.value() && outname.set()) 
    save_volume(pixdist,appendFSLfilename(outname.value(),"_initmask"));
 ...
 // read in images
  ...
  if (debug.value()) { 
    ...
    if (outname.set()) {   // save results
      ...
      save_volume4D(invwarp,appendFSLfilename(outname.value(),"_init"));
  ...
  if (debug.value()) { 
      if (fnirtfile.AbsOrRel()==RelativeWarps) { convertwarp_abs2rel(invwarp); }
      save_volume4D(invwarp,appendFSLfilename(outname.value(),"_prefill"));
      if (fnirtfile.AbsOrRel()==RelativeWarps) { convertwarp_rel2abs(invwarp); }
  }
  ...
  if (!nojaccon.value()) {
    ...
    if (debug.value()) { 
      if (fnirtfile.AbsOrRel()==RelativeWarps) { convertwarp_abs2rel(invwarp); }
      save_volume4D(invwarp,appendFSLfilename(outname.value(),"_postfill"));
      if (fnirtfile.AbsOrRel()==RelativeWarps) { convertwarp_rel2abs(invwarp); }
    }
...
//     constrain_topology(invwarp,jmin.value(),jmax.value());
  ...
  if (debug.value()) {
    ...
    save_volume4D(deriv,appendFSLfilename(outname.value(),"_deriv"));
    save_volume4D(costim,appendFSLfilename(outname.value(),"_costim"));
  }
