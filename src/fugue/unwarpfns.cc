...
{ 
  ...
  for (int z=zmin; z<=zmax; z++) {
    ...
    aslice = absmap.ROI(absmap.minx(),absmap.miny(),z,0,absmap.maxx(),absmap.maxy(),z,0);
    // find the mask
    mslice = make_head_mask(aslice,thresh);
    // set the corresponding slice of mask to mslice
    mask.copyROI(mslice,mask.minx(),mask.miny(),z,0,mask.maxx(),mask.maxy(),z,0);
  }
  return mask;
}
...
ColumnVector estimate_linear_ramps(const volume<float>& ph, const volume<float>& mask)
{
  ColumnVector ramp(3);
  ramp = 0.0;
				...
				int n, bool unique3Dlabels) 
{
  ...
  for (int z=zmin; z<=zmax; z++) {
    ...
    pslice = phasemap.ROI(phasemap.minx(),phasemap.miny(),z,0,phasemap.maxx(),phasemap.maxy(),z,0);
    mslice = mask.ROI(mask.minx(),mask.miny(),z,0,mask.maxx(),mask.maxy(),z,0);
    ...
    label.copyROI(lslice,label.minx(),label.miny(),z,0,label.maxx(),label.maxy(),z,0);
  }
  return label;
}
		       ...
		       const volume<int>& label, bool verbose)
{
  ...
  for (int z=zmin; z<=zmax; z++) {
    ...
    pslice = phasemap.ROI(phasemap.minx(),phasemap.miny(),z,0,phasemap.maxx(),phasemap.maxy(),z,0);
    lslice = label.ROI(label.minx(),label.miny(),z,0,label.maxx(),label.maxy(),z,0);
    copyconvert(lslice,mslice);
    mslice.binarise(0.5);
    // unwrap 
    uslice = unwrap(pslice,lslice,verbose);
    // set the corresponding slice of uphase to uslice
    uphase.copyROI(uslice,uphase.minx(),uphase.miny(),z,0,uphase.maxx(),uphase.maxy(),z,0);
  }
