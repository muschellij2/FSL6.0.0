...
//    }
  ...
  if (unwarpfname.set()) {
    ...
    for (int t0=invol.mint(); t0<=invol.maxt(); t0++) {
      volume<float> tmpInVol(invol[t0]);
      volume<float> tmpResVol(resvol[t0]);

      if (unwarpdir.set()) { 
	swapdirections(tmpInVol,unwarpdir_value); 
	swapdirections(pixshift,unwarpdir_value);       
      }      
      if (phaseconj.value()) {
	// Phase Conjugate method
	tmpResVol = unwarpPhaseConj1D(tmpInVol,fmap,dwellval);
      } else if (matrixinverse.value()) {
	// Matrix Inverse method
	tmpResVol = unwarpMatrixInverse1D(tmpInVol,fmap,dwellval);
      } else {
	// Pixel Shifting method (default)
	if (!icorronly.value()) {
	  tmpResVol = apply_pixshift(tmpInVol,pixshift);
	} else {
	  tmpResVol = tmpInVol;
	}
	if (icorr.value() || icorronly.value()) {
	  ...
	  derivshift += 1.0f;
	  tmpResVol *= derivshift;
	}
      }
      if (unwarpdir.set()) { 
	swapdirections(tmpResVol,unwarpdir_value); 
	swapdirections(pixshift,unwarpdir_value);       
      }     
      resvol[t0]=tmpResVol;
    }
  ...
  if (warpfname.set()) {
	...
	swapdirections(fmap,unwarpdir_value);       
      ...
      tmpResVol = warplikeepi1D(tmpInVol,fmap,dwellval,!nokspace.value());
      if (unwarpdir.set()) { 
	swapdirections(tmpResVol,unwarpdir_value); 
	swapdirections(fmap,unwarpdir_value);       
      }
