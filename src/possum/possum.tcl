...
proc possum:previewmotionplotdialog { w } {
    ...
    toplevel $w1
    set filenm $entries($w,userintmotfile)
    # used to be : set filenm $entries($w,mot)
...
#    catch { exec sh -c "${FSLDIR}/bin/imtest $filenm" } oval
	...
	puts "${POSSUMDIR}/bin/possum_plot.py $filenm $tmpnam"
	catch { exec sh -c "${POSSUMDIR}/bin/possum_plot.py $filenm $tmpnam" } oval
...
proc possum:previewmotionplot { w } {
    global entries guivars FSLDIR POSSUMDIR
    set convertcom "${FSLDIR}/bin/pngappend"
    set validim 0
    set width 100
    set filenm $entries($w,userintmotfile)
...
#	if { $sizeoffile > 10 } { set width 700 }
	...
	puts "${POSSUMDIR}/bin/possum_plot.py $filenm $tmpnam  ... 2"
	catch { exec sh -c "${POSSUMDIR}/bin/possum_plot.py $filenm $tmpnam" } oval
	puts $oval
...
proc possum:savescripts { w seqtype te tr trslc nx ny nz dx dy dz zstart angle nvol maxg riset bw gap slcdir phasedir readdir coverage plus pluss scriptpath outpath} {
	...
	if { $entries($w,motion_yn) == 1 } { set motion $entries($w,userintmotfile) } else { set motion "${FSLDIR}/data/possum/zeromotion" }
...
proc possum:updatemotion { w } {
    global entries guivars FSLDIR POSSUMDIR
    if { $entries($w,motion_yn) == 1 } {
	# run external script to generate interpolated file
	if { "X$entries($w,userintmotfile)" == "X" } {
	    catch { exec sh -c "${FSLDIR}/bin/tmpnam" } tmpnam
	    set entries($w,userintmotfile) $tmpnam
	}
	catch { exec sh -c "${POSSUMDIR}/bin/possum_interpmot.py $entries($w,motion_type) $entries($w,tr) $entries($w,trslc) $entries($w,outsize_nz) $entries($w,numvol) $entries($w,mot) $entries($w,userintmotfile)" } oval
	if { "X$oval" != "X" } {
	    puts $oval
	}
	pack $w.mot $w.mottypemenu $w.motpreview $w.motprevimg -in  $guivars($w,lfmotion) -side top -anchor w -padx 3 -pady 3
	set entries($w,slcsampfactor) 6
    } else {
	pack forget $w.mot $w.mottypemenu $w.motpreview $w.motprevimg $w.b0maintime.b0im.b0timewarn $w.motwarn
	set entries($w,slcsampfactor) 2
    }
    $w.nb compute_size
}
...
puts $channel "set entries(\$w,motion_type) \"$entries($w,motion_type)\""
puts $channel "set entries(\$w,userintmotfile) \"$entries($w,userintmotfile)\""
...
#    catch { exec sh -c "${FSLDIR}/bin/imcp $obvol $out/brain" } oval
    ...
    if { $mot != "${FSLDIR}/data/possum/zeromotion" } {
	catch { exec sh -c "cp $entries($w,mot) $out/motion_pre" } oval
    }
