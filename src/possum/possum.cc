...
//end
  ...
  read_volumeROI(phantom,opt_object.value(),myid,0,0,0,Nxx,-1,-1,-1,numprocs,1,1,1);
  ...
  if (opt_activation.set()) {
    ...
    read_volumeROI(activation4D,opt_activation4D.value(),myid,0,0,0,Nxx,-1,-1,-1,numprocs,1,1,1);
  ...
  cout<<""<<endl;
	  ...
	  if (level==3){
	    ...
	    int seg=(int)numpointstmp(2);
          if (seg==0 || seg>numpoints || save_kcoord==-1) seg=numpoints;
