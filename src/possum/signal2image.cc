...
//Tejas-end
	...
	for (int nn=1;nn<=n;nn++){
	  volume<double> kspace_real_temp(kspace_real[nn-1]);
	  volume<double> kspace_imag_temp(kspace_imag[nn-1]);
	  ...
	  for (int s=0;s<nslc;s++){
		...
		int xdir=l;
		setdir(xdir, ydir, zdir, l,k,s, readdir, phasedir, slcdir);
		kspace_real_temp(xdir,ydir,zdir)=kspace_real_temp(xdir,ydir,zdir)*W(xdir,ydir,zdir);
		kspace_imag_temp(xdir,ydir,zdir)=kspace_imag_temp(xdir,ydir,zdir)*W(xdir,ydir,zdir);
		kspaceHpc_real(xdir,ydir,zdir)=kspace_real(xdir,ydir,zdir)*Ws(xdir,ydir,zdir);
		kspaceHpc_imag(xdir,ydir,zdir)=kspace_imag(xdir,ydir,zdir)*Ws(xdir,ydir,zdir);
	      }
	    }
	  }
	  kspace_real_temp.swapdimensions(aa,bb,cc); 
	  kspace_imag_temp.swapdimensions(aa,bb,cc);
	  fftshift(kspace_real_temp);
	  fftshift(kspace_imag_temp);
	  fft2(kspace_real_temp,kspace_imag_temp);
	  fftshift(kspace_real_temp);
	  fftshift(kspace_imag_temp);
	  ...
	  for (int s=0;s<nslc;s++){
		...
		int xdir=l;
		setdir(xdir, ydir, zdir, l,k,s, readdir, phasedir, slcdir);
		kspace_real_temp(xdir,ydir,zdir)=pcA(xdir,ydir,zdir)*kspace_real_temp(xdir,ydir,zdir)-pcB(xdir,ydir,zdir)*kspace_imag_temp(xdir,ydir,zdir);
	      }
	    }
	  }
	  kspace_real_temp.swapdimensions(aaa,bbb,ccc); 
	  kspace_real[nn-1]=kspace_real_temp;
	...
	volume<double> kspace_imag_temp(kspace_imag[nn-1]);
        ...
        fftshift(kspace_imag_temp);
        kspace_real_temp.swapdimensions(aaa,bbb,ccc); 
        kspace_imag_temp.swapdimensions(aaa,bbb,ccc);
	kspace_real[nn-1]=kspace_real_temp;
	kspace_imag[nn-1]=kspace_imag_temp;	
      }
      if (useabs.value()) {
        volume4D<double> dummy(kspace_real);
        volume4D<double> dummy_phase(kspace_real);
