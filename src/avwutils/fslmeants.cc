...
int main(int argc,char *argv[])
{
  ...
  volume<float> mask, label, mask_nonbin;
  if (maskname.set()) {
  ...
  for (int iter=1; iter<=nlabs; iter++) {
	...
	for (int y=mask.miny(); y<=mask.maxy(); y++) {
	  for (int x=mask.minx(); x<=mask.maxx(); x++) {
		...
		ColumnVector v(4);
		v << x << y << z << 1.0;
		v = vin.niftivox2newimagevox_mat().i() * v;
		meants(1,num) = v(1);
		meants(2,num) = v(2);
		meants(3,num) = v(3);
