...
int applytopup()
{
  ...
  if (datatype.set()) {
    save_volume_dtype(ovol,outfname.value(),dtypecode);
  }
  else {
    save_volume_dtype(ovol,outfname.value(),NEWIMAGE::dtype(infnames[0]));
  }
                                         ...
                                         std::vector<NEWIMAGE::volume4D<float> >&         scans)
{
  ...
  for (unsigned int c=0; c<collections.NCollections(); c++) {
    ...
    for (int d=0; d<scans[0].tsize(); d++) {
      for (int sl=0; sl<scans[0].zsize(); sl++) {
	NEWIMAGE::ShadowVolume<float> tmpVol(ovol[d]);
        add_to_slice(tmpVol,mu[d][sl],sl,row);
      }
    }
                                         ...
                                         std::vector<NEWIMAGE::volume4D<float> >&         scans)
{
  ...
  for (unsigned int c=0; c<collections.NCollections(); c++) {
    ...
    for (int d=0; d<ovol.tsize(); d++) {
      ...
      for (int sl=0; sl<scans[0].zsize(); sl++) {
	NEWIMAGE::ShadowVolume<float> tmpVol(ovol[d]);
        add_to_slice(tmpVol,mu[sl],sl,row);
      }
