...
void do_work_bvars(){
  ...
  //**************** VERTEX ANALYSIS HERE *********************/
	...
	volscalar.addvolume(draw_scalar_mesh(refim,avMesh,ScalarVals.Column(n)));
      ...
      save_volume(maskvol,make_basename(outname.value())+"_mask");
      if (debug.value()) { save_volume4D(volnormals,make_basename(outname.value())+"_normals"); }
    }
	...
	newm++;
      ...
      write_ascii_matrix(make_basename(saveVertices.value())+"_mat.txt",ReshapedVerts);
