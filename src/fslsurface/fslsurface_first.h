...
namespace fslsurface_name {
  ...
  //  template<class T, class T2> 
	...
	void readBvars(const std::string & filename, const string & image_path, std::string & modelname, unsigned int & Nsubjects, std::vector<std::string> & v_imagenames, std::vector< std::vector<float> > & v_bvars, std::vector< std::vector<float> > & v_xfms);
    ...
    void reconSurfaces_from_bvars( std::vector< fslSurface<float,unsigned int> > & v_surf, const std::string & filename, const string & image_path, const string & template_name, std::string out_base = "NULL",  bool reconMNI=false );
