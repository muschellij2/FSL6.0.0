...
{
  /* {{{ vars */
int  picsetup=0, sagcorskip=0, axiskip=0, height=0;
size_t picsize=0;
/* }}} */
  /* {{{ first image stuff  */
 ...
 for (unsigned int current=0;current<inputOptions.size();current++ ) {
   ...
   if (strncmp(theopt,"-l",2)==0) { /* lut option lut */
	...
	edgethreshold = edgethresh*(edge.max()-edge.min()) + edge.min();
	 ...
	 //The magic number is taken as "worst case" ram requirement for slicer running on 
	 //this image. It doesn't check the particular options that have been passed
	 //to this function, 2*zsize seems too large, zsize should be the maximum, but 2*
	 //has worked for the last 15 years so it's not being changed!
