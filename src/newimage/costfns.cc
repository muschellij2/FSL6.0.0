...
#ifdef SAFE_FLIRT
      if ( (ix>=0) && (iy>=0) && (iz>=0) 
	   && (ix<v.xsize()-1) && (iy<v.ysize()-1) && (iz<v.zsize()-1) ) {
#endif
...
//    }
  ...
  ///////////////////////////////////////////////////////////////////////
				   ...
				   const int no_bins, const float smoothsize)
   {
	...
	numy[i]=0.0; sumy[i]=0.0;  sumy2[i]=0.0;
      ...
      for (int64_t z=0; z<vref.zsize(); z++) {
	for (int64_t y=0; y<vref.ysize(); y++) {
	  for (int64_t x=0; x<vref.xsize(); x++) {
	    //   assume vref and warpvol have same voxel size
	    //   look up the warp dest coordinate (in mm)
	    if (warpvol.in_bounds(x,y,z)) {
	      o1 = warpvol(x,y,z,0) / vtest.xdim();
	      o2 = warpvol(x,y,z,1) / vtest.ydim();
	      o3 = warpvol(x,y,z,2) / vtest.zdim();
  ...
  float Costfn::bbr(const Matrix& aff, const ColumnVector& nonlin_params, 
	...
	rvc(4)=1; tvc(4)=1;
	for (int refx=0; refx<refvol.xsize(); refx++) {
	  for (int refy=0; refy<refvol.ysize(); refy++) {
	    for (int refz=0; refz<refvol.zsize(); refz++) {
  ...
  int Costfn::set_bbr_seg(const volume<float>& bbr_seg) 
    {
      ...
      for (int z=0; z<wmseg.zsize(); z++) {
	for (int y=0; y<wmseg.ysize(); y++) {
	  for (int x=0; x<wmseg.xsize(); x++) {
