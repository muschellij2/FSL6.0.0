...
namespace NEWIMAGE {
 ...
 Matrix NewimageVox2NewimageVoxMatrix(const Matrix& flirt_in2ref,
  ...
  // DEBUG
  template <class T>
  void print_volume_info(const volume<T>& source, const string& name, ostream& output=std::cout) 
  {
    output << name << ":: Size = (" << source.xsize() << ","
	 << source.ysize() << "," << source.zsize() << "," << source.tsize() << ")" << endl;
    output << name << ":: Dims = (" << source.xdim() << ","
	 << source.ydim() << "," << source.zdim() << "," << source.tdim() << ")" << endl;
...
//default values for lut1 etc
 ...
 ///////////////////////////////////////////////////////////////////////////
  ...
  {
    volume<T> im(input);
    if (!samesize(input,mask)) { cerr << "ERROR::dilall::image are not the same size" << endl; }
					  ...
					  const string& interpmethod)
{
  ...
  vector<volume4D<float> > sampledData;
  vector<volume<float> > sampledMask;
						...
						const string& interpmethod)
{
  ...
  if ((interp>0) && (!samesize(bvol,valim.constSubVolume(0))))
    {  print_volume_info(bvol,string("bvol"),std::cerr); print_volume_info(mask,string("mask"),std::cerr); print_volume_info(valim.constSubVolume(0),string("valim"),std::cerr); 
