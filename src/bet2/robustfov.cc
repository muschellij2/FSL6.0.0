...
int do_work(int argc, char* argv[]) 
{
  ...
  for (int loopnum=1; loopnum<=2; loopnum++) {
    ...
    for (int q=qstart, row=rowmax; q!=qend; q+=qinc, row--) {
      if (abs(dim)==1) {
	copyv=vol.ROI(q,0,0,0,q,vol.ysize()-1,vol.zsize()-1,vol.tsize()-1);
      } else if (abs(dim)==2) {
	copyv=vol.ROI(0,q,0,0,vol.xsize()-1,q,vol.zsize()-1,vol.tsize()-1);
      } else if (abs(dim)==3) {
	copyv=vol.ROI(0,0,q,0,vol.xsize()-1,vol.ysize()-1,q,vol.tsize()-1);
      }
      float p1, p99, thresh;
      if (loopnum==1) { 
	copyv.binarise(0,copyv.max()+1,exclusive);
	nzsum(row)=copyv.sum(); 
      }
      if (loopnum==2) {
	if (nzsum(row)>0) {
  ...
  if (roivol.set()) {
    volume<float> roiv = vol.ROI(MISCMATHS::round(fov(1)),MISCMATHS::round(fov(3)),MISCMATHS::round(fov(5)),0,
				 MISCMATHS::round(fov(1)+fov(2))-1,MISCMATHS::round(fov(3)+fov(4))-1,MISCMATHS::round(fov(5)+fov(6))-1,vol.maxt());
