...
// http://stackoverflow.com/questions/5255806/how-to-calculate-tangent-and-binormal
  ...
  newmesh calculate_strains(double fit_radius, const NEWMESH::newmesh & ORIG, const NEWMESH::newmesh & FINAL, const boost::shared_ptr<Matrix> &PrincipalStretches,const boost::shared_ptr<RELATIONS> &REL){
	...
	if(dist<=fit_radius && dir_chk1>=0 && dir_chk2>=0){
 ...
 double calculate_triangular_strain(const Triangle & ORIG_tr, const Triangle & FINAL_tr, const double & mu, const double &kappa,  const boost::shared_ptr<ColumnVector> & indexSTRAINS){
    ...
    W=triangle_strain(ORIG2D,FINAL2D,mu,kappa,indexSTRAINS);
 
  
    return W;
  }

  double calculate_triangular_strain(int index, const NEWMESH::newmesh & ORIG, const NEWMESH::newmesh & FINAL, const double & mu, const double &kappa,  const boost::shared_ptr<ColumnVector> & indexSTRAINS){
    ...
    W=triangle_strain(ORIG2D,FINAL2D,mu,kappa,indexSTRAINS,false);
  ...
  double triangle_strain(const Matrix& AA, const Matrix & BB, const double &MU,const double &KAPPA, const boost::shared_ptr<ColumnVector> & strains, bool report){
    ...
    if(strains.get()){
      ...
      if( (*strains)(1)>100 && report==true){
	cout << " all t0 distances" <<  c0 << " " << c1 << " " << c2 << " " << c3 << " " << c4 << " " << c5 << endl;
	cout << " all tf distances" <<  c0c << " " << c1c << " " << c2c << " " << c3c << " " << c4c << " " << c5c << endl;
	cout << " F " << endl;
	cout << F << endl; 
	cout << " edges " << endl;
	cout << edges << endl;
	...
	cout << " I1 " << I1 << " " << I3 << " " << I1st  << " J " << J  << " mu " << MU << " kappa " << KAPPA <<   " W " << W << endl;
	cout << " B " << B1 << " " << B2 << " " << B3 << " e11 " << e11 << " " << e22 << " " <<  e12 << " Z " << X << " " << Y << " strains " << 	X/2+sqrt((Y/2)*(Y/2)+(e12)*(e12))  << " " << X/2-sqrt((Y/2)*(Y/2)+(e12)*(e12)) <<  endl;
      }
    }
  ...
  bool set_data(const  string &dataname,boost::shared_ptr<BFMatrix> &BF,NEWMESH::newmesh &M, bool issparse){
    ...
    filetype=meshFileType(dataname); // will detect file type from ending, data matrices need to be ended .txt at the moment
